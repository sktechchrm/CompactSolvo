...
export default function Navigation() {
  ...
  const activeModule = hrModules.find(m => m.id === currentPage || location.pathname.includes(m.id));
  ...
  useEffect(() => { setIsMobileMenuOpen(false); setIsHRMenuOpen(false); }, [location.pathname]);

  const handleSignOut   = () => { logout(); navigate("/"); };
  const MODULE_ROUTES: Record<string, string> = {
    maternity:'/maternity', settlement:'/settlement', leftnotice:'/left-notice',
    personalfile:'/employees', requisition:'/requisition', increment:'/increment',
    meeting:'/meeting', workerrights:'/worker-rights', workerguideline:'/worker-rights',
    grievance:'/grievance', reports:'/reports', authority:'/authority', database:'/admin/database',
  };
  const navTo = (page: string) => { navigate(MODULE_ROUTES[page] ?? `/${page}`); setIsHRMenuOpen(false); setIsMobileMenuOpen(false); };
